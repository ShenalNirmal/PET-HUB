 ...
 if (isset($_GET['id']) && !empty($_GET['id'])) {
     ...
     if ($result->num_rows > 0) {
         ...
         // Product with the given ID not found, handle accordingly (e.g., redirect)
         echo "<script>alert('Product not found'); window.location='edit-product.php';</script>";
         exit;
     }
 } else {
     // ID not provided in the GET parameters, handle accordingly (e.g., redirect)
     echo "<script>alert('Invalid product ID'); window.location='edit-product.php';</script>";
     exit;
 }
 ...
 if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
   ...
   if (!empty($_FILES["image"]["name"]) && in_array($imageFileType, $allowedExtensions)) {
       if (move_uploaded_file($_FILES["image"]["tmp_name"], $imagePath)) {
           ...
           $updateSql = "UPDATE products SET name='$productName', price='$price', image_path='$imagePath', category='$category' WHERE id=$id";
       ...
       $updateSql = "UPDATE products SET name='$productName', price='$price', category='$category' WHERE id=$id";
                     ...
                     <div class="card-body product-form">
                        ...
                        <form method="POST" enctype="multipart/form-data">
                           <div class="form-group w-50">
                              ...
                              <input type="text" name="name" class="form-control" value="<?php echo $value['name'] ?>" placeholder="Enter Product Name" required>
                           </div>
                           <div class="form-group w-50">
                           ...
                           <div class="form-group w-50">
                              ...
                              <input type="text" name="category" class="form-control" value="<?php echo $value['category'] ?>" placeholder="Enter Category" required>
                           </div>
                           <div class="cat-btn">
